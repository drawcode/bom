...
public class Message : BaseEntity {
    ...
    // profile conversions.
    public  profile_from_name { get; set; }
    public  read { get; set; }
    public  profile_from_id { get; set; }
    public  profile_to_token { get; set; }
    public  app_id { get; set; }
    public  profile_to_id { get; set; }
    public  profile_to_name { get; set; }
    public  data { get; set; }
    public  sent { get; set; }
	...
	if(dict.ContainsKey("profile_from_name")) {
	    if(dict["profile_from_name"] != null) {
	    	profile_from_name = DataType.Instance.Fill(dict["profile_from_name"]);
	    }		
	}
	if(dict.ContainsKey("read")) {
	    if(dict["read"] != null) {
	    	read = DataType.Instance.Fill(dict["read"]);
	    }		
	}
	if(dict.ContainsKey("profile_from_id")) {
	    if(dict["profile_from_id"] != null) {
	    	profile_from_id = DataType.Instance.Fill(dict["profile_from_id"]);
	    }		
	}
	if(dict.ContainsKey("profile_to_token")) {
	    if(dict["profile_to_token"] != null) {
	    	profile_to_token = DataType.Instance.Fill(dict["profile_to_token"]);
	    }		
	}
	...
	if(dict.ContainsKey("profile_to_id")) {
	    if(dict["profile_to_id"] != null) {
	    	profile_to_id = DataType.Instance.Fill(dict["profile_to_id"]);
	    }		
	}
	if(dict.ContainsKey("profile_to_name")) {
	    if(dict["profile_to_name"] != null) {
	    	profile_to_name = DataType.Instance.Fill(dict["profile_to_name"]);
	    }		
	}
	...
	if(dict.ContainsKey("sent")) {
	    if(dict["sent"] != null) {
	    	sent = DataType.Instance.Fill(dict["sent"]);
	    }		
	}
	if(dict.ContainsKey("subject")) {
	    if(dict["subject"] != null) {
	    	subject = DataType.Instance.Fill(dict["subject"]);
	    }		
	}
    }
}
