...
public class ProfileDevice : BaseEntity {
    ...
    public  device_version { get; set; }
    public  device_type { get; set; }
    public  device_os { get; set; }
    public  device_platform { get; set; }
    public  device_id { get; set; }
	...
	if(dict.ContainsKey("device_version")) {
	    if(dict["device_version"] != null) {
	    	device_version = DataType.Instance.Fill(dict["device_version"]);
	    }		
	}
	if(dict.ContainsKey("device_type")) {
	    if(dict["device_type"] != null) {
	    	device_type = DataType.Instance.Fill(dict["device_type"]);
	    }		
	}
	if(dict.ContainsKey("device_os")) {
	    if(dict["device_os"] != null) {
	    	device_os = DataType.Instance.Fill(dict["device_os"]);
	    }		
	}
	if(dict.ContainsKey("device_platform")) {
	    if(dict["device_platform"] != null) {
	    	device_platform = DataType.Instance.Fill(dict["device_platform"]);
	    }		
	}
	if(dict.ContainsKey("device_id")) {
	    if(dict["device_id"] != null) {
	    	device_id = DataType.Instance.Fill(dict["device_id"]);
	    }		
	}
    }
}
